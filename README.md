...
> Los siguientes Epics agrupan las User Stories y Technical Stories del sistema Pawtient. Cada Epic representa una capacidad de negocio o técnica de alto nivel.

<br>

| Epic ID | Título | Descripción | Historias |
|:---:|:---|:---|:---:|
| **EP01** | Autenticación y Gestión de Usuarios | Proveer un sistema seguro de registro, inicio de sesión y gestión de roles para administradores de clínica / veterinarios independientes y dueños de mascotas. | US01–US03 |
| **EP02** | Gestión de Historiales Clínicos | Centralizar los registros médicos de las mascotas en formato digital, eliminando el uso de historias en papel o Excel y permitiendo el acceso desde dispositivos móviles. | US04–US07 |
| **EP03** | Programación y Gestión de Citas | Digitalizar y automatizar la agenda de citas para reducir el estrés del personal clínico y mejorar la experiencia del dueño de mascota. | US08–US11 |
| **EP04** | Trazabilidad de Suministros e Inventario | Proveer control en tiempo real del inventario de medicamentos e insumos para evitar faltantes, vencimientos y errores operativos en la clínica. | US12–US15 |
| **EP05** | Perfiles y Gestión de Mascotas | Permitir a los dueños gestionar el perfil digital de sus mascotas con historial, vacunas y documentos accesibles desde el celular. | US16–US18 |
| **EP06** | Notificaciones y Recordatorios Automatizados | Automatizar el envío de alertas para vacunas, citas y cuidados preventivos tanto al equipo clínico como a los dueños de mascotas. | US19–US21 |
| **EP07** | Landing Page de Pawtient | Presentar la propuesta de valor de Pawtient diferenciando el mensaje por segmento y facilitando el registro o contacto con la plataforma. | US22–US25 |
| **EP08** | Technical Stories – API Real (Backend) | Proveer endpoints seguros, documentados y funcionales con Spring Boot que soporten todas las operaciones del sistema Pawtient. | TS01–TS05 |
...
| **EP10** | Technical Stories – Frontend Angular | Implementar componentes, servicios y rutas en Angular que consuman la API y soporten las funcionalidades de la aplicación web. | TS10–TS14 |

<br>

### User Stpries & Technical Stories

> Las User Stories fueron definidas a partir del análisis de entrevistas realizadas a los dos segmentos objetivo: **administradores de clínica veterinaria / veterinarios independientes** y **dueños de mascotas**. Se incluyen User Stories para la Landing Page y Technical Stories para el API real (backend Spring Boot), la fake API (JSON Server) y el frontend Angular. Los criterios de aceptación siguen la estructura Gherkin (Given–When–Then) con tres escenarios por historia.

<br>

| Story ID | Título | Descripción | Criterios de Aceptación | Epic |
|:---:|:---|:---|:---|:---:|
| US01 | Registrar cuenta como administrador de clínica o veterinario independiente | Como administrador de clínica o veterinario independiente, quiero crear una cuenta en Pawtient con mis datos profesionales para acceder a las funcionalidades de gestión clínica. | **Escenario 1 – Registro exitoso:** Given el usuario completa todos los campos obligatorios (nombre completo, correo, contraseña, número de colegiatura y tipo de perfil: clínica o independiente), When envía el formulario de registro, Then el sistema crea la cuenta, envía un correo de verificación y muestra un mensaje de confirmación. **Escenario 2 – Correo ya registrado:** Given el usuario ingresa un correo electrónico que ya existe en el sistema, When envía el formulario, Then el sistema muestra el mensaje "El correo ya está registrado" y no crea una cuenta duplicada. **Escenario 3 – Contraseña débil:** Given el usuario ingresa una contraseña que no cumple los requisitos mínimos de seguridad, When envía el formulario, Then el sistema resalta los requisitos incumplidos y no permite que el registro continúe. | EP01 |
| US02 | Iniciar sesión en la plataforma | Como usuario registrado (administrador de clínica, veterinario independiente o dueño de mascota), quiero iniciar sesión con mis credenciales para acceder a las funcionalidades según mi perfil. | **Escenario 1 – Inicio de sesión exitoso:** Given el usuario ingresa un correo y contraseña válidos, When confirma el inicio de sesión, Then el sistema autentica al usuario, genera un token de sesión y lo redirige al panel correspondiente a su perfil. **Escenario 2 – Credenciales incorrectas:** Given el usuario ingresa una contraseña incorrecta, When intenta iniciar sesión, Then el sistema muestra el mensaje "Credenciales inválidas" sin especificar cuál campo es incorrecto y deniega el acceso. **Escenario 3 – Cuenta no verificada:** Given el usuario registrado no ha verificado su correo electrónico, When intenta iniciar sesión, Then el sistema informa que la cuenta requiere verificación y ofrece reenviar el correo de confirmación. | EP01 |
| US03 | Recuperar contraseña olvidada | Como usuario registrado, quiero recuperar el acceso a mi cuenta en caso de olvidar mi contraseña para no perder el acceso a la plataforma. | **Escenario 1 – Correo de recuperación enviado:** Given el usuario ingresa su correo registrado en el formulario de recuperación, When confirma la solicitud, Then el sistema envía un enlace de restablecimiento de contraseña válido por 30 minutos. **Escenario 2 – Correo no registrado:** Given el usuario ingresa un correo que no existe en el sistema, When confirma la solicitud, Then el sistema muestra el mensaje genérico "Si el correo está registrado, recibirás un enlace" sin confirmar ni negar la existencia de la cuenta. **Escenario 3 – Enlace expirado:** Given el usuario intenta usar un enlace de recuperación después de los 30 minutos de validez, When accede al enlace, Then el sistema indica que el enlace ha expirado y permite solicitar uno nuevo. | EP01 |
| US04 | Registrar historia clínica de una mascota | Como administrador de clínica o veterinario independiente, quiero crear y guardar la historia clínica digital de una mascota para eliminar el uso de registros físicos y acceder a la información desde cualquier dispositivo. | **Escenario 1 – Registro exitoso:** Given el profesional completa todos los campos obligatorios (nombre de mascota, especie, raza, peso, diagnóstico, tratamiento y fecha), When confirma el guardado, Then el sistema almacena el registro y muestra un mensaje de confirmación. **Escenario 2 – Campos incompletos:** Given el profesional intenta guardar una historia clínica con campos obligatorios vacíos, When confirma el guardado, Then el sistema resalta los campos faltantes y no permite continuar hasta completarlos. **Escenario 3 – Acceso desde dispositivo móvil:** Given el profesional accede al sistema desde un dispositivo móvil, When crea o consulta una historia clínica, Then el sistema muestra el formulario completo y funcional sin pérdida de campos ni funcionalidades. | EP02 |
...
| US06 | Editar y actualizar historia clínica | Como administrador de clínica o veterinario independiente, quiero actualizar la historia clínica de una mascota después de cada consulta para mantener la información vigente y con trazabilidad de cambios. | **Escenario 1 – Actualización exitosa:** Given el profesional abre un registro existente y modifica el diagnóstico o tratamiento, When guarda los cambios, Then el sistema registra la actualización incluyendo la fecha, hora y nombre del usuario que realizó el cambio. **Escenario 2 – Edición sin permisos:** Given un dueño de mascota intenta editar una historia clínica desde su perfil, When intenta acceder al modo de edición, Then el sistema deniega la acción y muestra el mensaje "No tienes permisos para editar este registro". **Escenario 3 – Historial de cambios visible:** Given el profesional revisa una historia clínica, When accede a la sección de trazabilidad, Then el sistema muestra un registro cronológico de todas las modificaciones realizadas con nombre del autor y fecha. | EP02 |
...
| US08 | Agendar cita desde la clínica o consultorio | Como administrador de clínica o veterinario independiente, quiero registrar una nueva cita en el sistema para organizar la agenda diaria sin depender de llamadas manuales ni registros en papel. | **Escenario 1 – Cita registrada correctamente:** Given el profesional selecciona fecha, hora y mascota del cliente, When confirma el registro, Then el sistema añade la cita al calendario y envía automáticamente una notificación al dueño de la mascota. **Escenario 2 – Horario no disponible:** Given el profesional selecciona un horario ya ocupado, When intenta confirmar, Then el sistema indica la no disponibilidad y sugiere los próximos horarios libres. **Escenario 3 – Cita fuera del horario de atención:** Given el profesional intenta registrar una cita fuera del horario configurado, When intenta confirmar, Then el sistema muestra una advertencia indicando que el horario seleccionado está fuera del rango configurado. | EP03 |
| US09 | Agendar cita como dueño de mascota | Como dueño de mascota, quiero solicitar una cita veterinaria desde mi celular para evitar llamadas telefónicas y reducir el tiempo invertido en coordinar turnos. | **Escenario 1 – Solicitud enviada exitosamente:** Given el dueño selecciona una clínica o veterinario, una de sus mascotas registradas y una fecha y hora disponibles, When confirma la solicitud, Then el sistema registra la cita y envía una confirmación mediante notificación push. **Escenario 2 – Sin horarios disponibles:** Given el dueño selecciona una fecha en la que todos los horarios están ocupados, When intenta avanzar, Then el sistema muestra el mensaje "No hay horarios disponibles para esta fecha" y permite seleccionar otra. **Escenario 3 – Cancelación con anticipación:** Given el dueño tiene una cita confirmada y la cancela con al menos 2 horas de anticipación, When confirma la cancelación, Then el sistema libera el horario, notifica al profesional y elimina los recordatorios asociados. | EP03 |
...
| US11 | Recibir recordatorios de citas | Como dueño de mascota, quiero recibir recordatorios automáticos antes de las citas de mi mascota para no olvidar los turnos médicos. | **Escenario 1 – Recordatorio 24 horas antes:** Given existe una cita confirmada, When faltan 24 horas para la cita, Then el sistema envía una notificación push al dueño con la fecha, hora, nombre de la clínica o profesional y motivo de consulta. **Escenario 2 – Recordatorio 1 hora antes:** Given existe una cita confirmada, When falta 1 hora para la cita, Then el sistema envía un segundo recordatorio de confirmación al dueño. **Escenario 3 – Recordatorios desactivados por el usuario:** Given el dueño ha desactivado los recordatorios de citas en su configuración, When el sistema evalúa el envío de una alerta, Then no envía ninguna notificación para ese tipo de evento. | EP03 |
| US12 | Registrar ingreso de suministros | Como administrador de clínica o veterinario independiente, quiero registrar el ingreso de nuevos suministros al inventario para mantener el stock actualizado en tiempo real. | **Escenario 1 – Ingreso registrado exitosamente:** Given el profesional completa los campos de nombre del producto, cantidad, unidad, fecha de vencimiento y proveedor, When confirma el registro, Then el sistema actualiza el stock y registra la transacción con fecha y usuario responsable. **Escenario 2 – Producto ya existente en inventario:** Given el profesional registra un suministro con el mismo nombre que uno existente, When confirma el ingreso, Then el sistema suma la cantidad al stock existente sin crear un registro duplicado. **Escenario 3 – Alerta de producto próximo a vencer:** Given un producto tiene fecha de vencimiento dentro de los próximos 30 días, When el profesional lo registra, Then el sistema genera una alerta visible en el panel principal indicando la proximidad del vencimiento. | EP04 |
...
| US14 | Registrar consumo de suministros por consulta | Como administrador de clínica o veterinario independiente, quiero registrar los insumos utilizados durante una consulta para descontarlos automáticamente del inventario y mantener la trazabilidad de uso. | **Escenario 1 – Descuento automático de inventario:** Given el profesional registra los insumos usados al cerrar una consulta, When guarda el registro, Then el sistema descuenta las cantidades del inventario y vincula la transacción al historial clínico de la mascota. **Escenario 2 – Cantidad supera el stock disponible:** Given el profesional intenta registrar una cantidad que supera el stock actual, When intenta guardar, Then el sistema muestra una alerta de stock insuficiente y no permite completar el registro hasta corregir la cantidad. **Escenario 3 – Reporte de consumo mensual:** Given el profesional accede al módulo de reportes y selecciona el período mensual, When genera el reporte, Then el sistema presenta un detalle de consumo por insumo con cantidad total utilizada y costo estimado. | EP04 |
...
| US16 | Crear perfil digital de mascota | Como dueño de mascota, quiero registrar el perfil de mi mascota en la plataforma para centralizar su información médica y acceder a ella en cualquier momento. | **Escenario 1 – Perfil creado exitosamente:** Given el dueño completa los campos de nombre, especie, raza, fecha de nacimiento y foto, When confirma el registro, Then el sistema crea el perfil y lo muestra en la sección "Mis mascotas" de la cuenta. **Escenario 2 – Registro de múltiples mascotas:** Given el dueño desea registrar una segunda mascota, When selecciona "Agregar mascota" y completa los datos, Then el sistema crea un perfil independiente y lo lista junto a las mascotas previamente registradas. **Escenario 3 – Historial visible desde el perfil:** Given el dueño abre el perfil de una mascota, When navega a la sección de historial, Then el sistema muestra todas las consultas anteriores ordenadas por fecha con diagnóstico y nombre del profesional tratante. | EP05 |
| US17 | Compartir historial médico con una clínica o veterinario | Como dueño de mascota, quiero compartir el historial de mi mascota con un nuevo profesional para que cuente con información previa relevante antes de la consulta. | **Escenario 1 – Compartir mediante código temporal:** Given el dueño genera un código de acceso temporal desde el perfil de su mascota, When el profesional ingresa ese código en el sistema, Then el sistema muestra el historial clínico completo durante el período de validez de 24 horas. **Escenario 2 – Código expirado:** Given el profesional intenta usar un código de acceso después de las 24 horas de validez, When el sistema valida el código, Then muestra el mensaje "Código expirado" y solicita al dueño generar uno nuevo. **Escenario 3 – Acceso revocado por el dueño:** Given el dueño compartió el historial con un profesional, When revoca el acceso desde su configuración, Then el sistema invalida el código de forma inmediata y el profesional ya no puede visualizar el historial. | EP05 |
| US18 | Buscar clínicas y veterinarios cercanos | Como dueño de mascota, quiero buscar clínicas veterinarias y veterinarios independientes cercanos con sus reseñas y servicios para elegir el mejor lugar donde atender a mi mascota. | **Escenario 1 – Búsqueda por ubicación exitosa:** Given el dueño activa la búsqueda de profesionales cercanos, When el sistema detecta su ubicación, Then muestra un listado de clínicas y veterinarios ordenados por distancia con nombre, dirección, calificación y servicios disponibles. **Escenario 2 – Sin resultados en el área:** Given el sistema no encuentra resultados dentro del radio de búsqueda predeterminado, When presenta los resultados, Then muestra el mensaje "No se encontraron clínicas ni veterinarios en tu área" y ofrece ampliar el radio de búsqueda. **Escenario 3 – Filtro por tipo de servicio:** Given el dueño aplica un filtro por tipo de servicio (urgencias, vacunación, grooming), When el sistema procesa el filtro, Then muestra únicamente los profesionales que ofrecen ese servicio específico. | EP05 |
| US19 | Recibir recordatorios de vacunación | Como dueño de mascota, quiero recibir alertas cuando la vacuna de mi mascota esté próxima para no perder el calendario de vacunación. | **Escenario 1 – Alerta 7 días antes de la fecha:** Given el calendario de vacunación tiene una dosis programada, When faltan 7 días para la fecha de aplicación, Then el sistema envía una notificación push al dueño con el nombre de la vacuna y un enlace para agendar la cita. **Escenario 2 – Alerta el día de la vacuna:** Given es el día en que corresponde aplicar una vacuna programada, When el sistema ejecuta la revisión diaria, Then envía un recordatorio urgente al dueño indicando que la vacuna corresponde a ese día. **Escenario 3 – Recordatorio cancelado tras aplicación:** Given el profesional registra la vacuna como aplicada, When el sistema actualiza el historial clínico, Then cancela todos los recordatorios pendientes de esa dosis y programa el siguiente según el esquema de dosis. | EP06 |
| US20 | Configurar preferencias de notificación | Como dueño de mascota, quiero personalizar el tipo y frecuencia de notificaciones que recibo para gestionar las alertas según mis necesidades. | **Escenario 1 – Configuración guardada exitosamente:** Given el dueño accede a los ajustes de notificaciones y selecciona los tipos de alerta deseados, When guarda los cambios, Then el sistema aplica las nuevas preferencias en todos los envíos posteriores. **Escenario 2 – Todas las notificaciones desactivadas:** Given el dueño desactiva todas las notificaciones desde su configuración, When el sistema intenta enviar cualquier alerta, Then no realiza ningún envío hasta que las preferencias sean reactivadas manualmente. **Escenario 3 – Configuración por defecto restaurada:** Given el dueño selecciona "Restaurar valores por defecto" en la configuración de notificaciones, When el sistema procesa la acción, Then habilita todas las categorías de notificación con las frecuencias estándar predefinidas. | EP06 |
| US21 | Recibir notificaciones de resultados y seguimiento | Como dueño de mascota, quiero recibir notificaciones cuando el profesional actualice el estado de seguimiento de mi mascota para mantenerme informado después de cada consulta. | **Escenario 1 – Notificación de actualización de seguimiento:** Given el profesional añade una nota de seguimiento al historial de una mascota, When el sistema confirma el guardado, Then envía una notificación push al dueño indicando que hay una actualización disponible en el perfil de su mascota. **Escenario 2 – Notificación de resultados de examen:** Given el profesional registra los resultados de un examen de laboratorio vinculado a una consulta, When confirma el registro, Then el sistema notifica al dueño que los resultados están disponibles para su revisión. **Escenario 3 – Historial de notificaciones accesible:** Given el dueño desea revisar notificaciones previas, When accede al centro de notificaciones, Then el sistema muestra el historial de los últimos 30 días ordenado cronológicamente con estado de lectura. | EP06 |
| US22 | Propuesta de valor para clínicas y veterinarios independientes | Como visitante del segmento clínica o veterinario independiente, quiero ver los beneficios del sistema en la landing page para evaluar si Pawtient se adapta a las necesidades de mi práctica profesional. | **Escenario 1 – Sección de beneficios visible:** Given el visitante accede a la landing page y navega a la sección dirigida a profesionales, When la sección carga, Then se visualizan al menos tres beneficios clave (gestión de historiales, agenda digital, control de inventario) con íconos y descripciones claras. **Escenario 2 – Llamada a la acción funcional:** Given el visitante revisa la sección de profesionales, When activa la opción "Solicitar demo" o "Registrar mi clínica", Then el sistema lo lleva al formulario de registro sin recargar la página completa. **Escenario 3 – Visualización en dispositivos móviles:** Given el visitante accede a la landing page desde un smartphone, When navega por la sección de beneficios para profesionales, Then el contenido se adapta correctamente a la pantalla sin desbordamientos ni pérdida de información. | EP07 |
| US23 | Propuesta de valor para dueños de mascotas | Como visitante del segmento dueño de mascota, quiero entender cómo Pawtient me ayuda a cuidar mejor a mi mascota para decidir si me registro en la plataforma. | **Escenario 1 – Sección para dueños visible:** Given el visitante llega a la landing page, When navega hasta la sección dirigida a dueños de mascotas, Then visualiza los beneficios principales (recordatorios de vacunas, historial digital, búsqueda de clínicos) con lenguaje accesible y no técnico. **Escenario 2 – Testimonios visibles:** Given el visitante llega a la sección de testimonios, When la página carga completamente, Then se muestran al menos dos testimonios de dueños de mascotas con nombre, tipo de mascota y valoración. **Escenario 3 – Registro desde la landing page:** Given el visitante revisa la sección de dueños y decide registrarse, When activa el botón de registro, Then el sistema lo redirige al formulario de creación de cuenta como dueño de mascota. | EP07 |
| US24 | Navegar entre secciones de la landing page | Como visitante de la landing page, quiero usar el menú de navegación para desplazarme rápidamente a cualquier sección y conocer todo el contenido disponible. | **Escenario 1 – Desplazamiento suave entre secciones:** Given el visitante selecciona un elemento del menú de navegación, When el enlace apunta a una sección dentro de la misma página, Then el sistema ejecuta un desplazamiento suave hasta esa sección sin recargar la página. **Escenario 2 – Menú visible durante el scroll:** Given el visitante hace scroll hacia abajo, When supera la altura del encabezado inicial, Then el menú permanece fijo en la parte superior de la pantalla y se mantiene accesible durante la navegación. **Escenario 3 – Sección activa resaltada en el menú:** Given el visitante se encuentra en una sección específica de la página, When el scroll posiciona esa sección como la principal visible en pantalla, Then el ítem correspondiente del menú se resalta visualmente para indicar la ubicación actual. | EP07 |
| US25 | Ver preguntas frecuentes en la landing page | Como visitante de la landing page, quiero consultar las preguntas frecuentes de la plataforma para resolver dudas antes de registrarme sin necesidad de contactar al equipo de soporte. | **Escenario 1 – Sección FAQ visible y accesible:** Given el visitante navega a la sección de preguntas frecuentes, When la sección carga, Then el sistema muestra al menos 6 preguntas organizadas por categoría (clínicas / profesionales y dueños de mascotas). **Escenario 2 – Expansión de respuesta:** Given el visitante selecciona una pregunta del listado, When la activa, Then el sistema expande la respuesta correspondiente sin redirigir a otra página. **Escenario 3 – Enlace de contacto al final de la sección:** Given el visitante revisa las preguntas frecuentes y no encuentra respuesta a su duda, When llega al final de la sección, Then el sistema muestra un enlace o formulario de contacto para consultas adicionales. | EP07 |
| TS01 | Endpoint de autenticación de usuarios | Como developer backend, quiero implementar un endpoint POST /api/v1/auth/login en Spring Boot que valide credenciales y devuelva un token JWT para que el frontend gestione sesiones de forma segura. | **Escenario 1 – Autenticación exitosa (200):** Given el developer envía una petición POST con email y password válidos, When el servidor valida las credenciales en la base de datos, Then retorna HTTP 200 con un token JWT firmado, el perfil del usuario (dueño o profesional) y el tiempo de expiración. **Escenario 2 – Credenciales inválidas (401):** Given el developer envía credenciales incorrectas, When el servidor valida el body, Then retorna HTTP 401 con el mensaje "Invalid credentials" sin especificar cuál campo es incorrecto. **Escenario 3 – Body malformado (400):** Given el developer envía una petición sin los campos requeridos, When el servidor valida el body, Then retorna HTTP 400 con el mensaje "Missing required fields" y la lista de campos faltantes. | EP08 |
| TS02 | Endpoint de creación de historia clínica | Como developer backend, quiero implementar un endpoint POST /api/v1/records en Spring Boot que persista historias clínicas en la base de datos para que el frontend registre consultas desde cualquier cliente. | **Escenario 1 – Creación exitosa (201):** Given el developer envía una petición POST con body válido (petId, diagnosis, treatment, date, professionalId) y token JWT vigente, When el servidor procesa la solicitud y persiste el registro, Then retorna HTTP 201 con el objeto creado incluyendo el id generado y la marca de tiempo. **Escenario 2 – Campos obligatorios faltantes (400):** Given el developer envía una petición POST con campos obligatorios ausentes, When el servidor valida el body con Bean Validation, Then retorna HTTP 400 con el mensaje "Missing required fields" y la lista de campos faltantes. **Escenario 3 – Token inválido o ausente (401):** Given el developer envía la petición sin token o con uno expirado, When el servidor valida la cabecera Authorization, Then retorna HTTP 401 con el mensaje "Unauthorized". | EP08 |
| TS03 | Endpoint de programación de citas | Como developer backend, quiero implementar un endpoint POST /api/v1/appointments en Spring Boot que registre citas verificando disponibilidad de forma transaccional para que el frontend web y la app móvil agenden turnos de forma unificada. | **Escenario 1 – Cita creada exitosamente (201):** Given el developer envía una petición POST con petId, professionalId, date y time válidos, When el servidor verifica la disponibilidad y persiste la cita, Then retorna HTTP 201 con el objeto de cita incluyendo el appointmentId y el estado "confirmed". **Escenario 2 – Conflicto de horario (409):** Given el developer envía una solicitud para un horario ya ocupado, When el servidor valida la disponibilidad, Then retorna HTTP 409 con el mensaje "Time slot not available" y los próximos horarios disponibles. **Escenario 3 – Fecha en el pasado (422):** Given el developer envía una fecha anterior a la fecha actual del servidor, When el servidor valida el campo date, Then retorna HTTP 422 con el mensaje "Invalid or past date". | EP08 |
| TS04 | Endpoints de gestión de inventario | Como developer backend, quiero implementar endpoints GET /api/v1/inventory y POST /api/v1/inventory en Spring Boot para que el módulo de trazabilidad de suministros opere correctamente. | **Escenario 1 – Listado de inventario exitoso (200):** Given el developer envía una petición GET con token válido y clinicId o professionalId como parámetro, When el servidor consulta los suministros, Then retorna HTTP 200 con el listado incluyendo nombre, stock actual, unidad de medida y fecha de vencimiento. **Escenario 2 – Ingreso de suministro exitoso (201):** Given el developer envía una petición POST con name, quantity, unit, expirationDate y el id del profesional válidos, When el servidor procesa la solicitud, Then retorna HTTP 201 con el registro actualizado; si el producto ya existe, retorna el stock acumulado. **Escenario 3 – Flag de stock bajo en la respuesta:** Given el developer consulta el inventario y algún suministro tiene stock igual o menor al mínimo configurado, When el servidor retorna la lista, Then incluye el campo `lowStock: true` en los objetos de suministros que cumplen esa condición. | EP08 |
| TS05 | Endpoint de notificaciones push | Como developer backend, quiero implementar un endpoint POST /api/v1/notifications/send en Spring Boot que encole y dispare notificaciones push para que el sistema automatice recordatorios de vacunas y citas. | **Escenario 1 – Notificación encolada exitosamente (200):** Given el developer envía una petición POST con userId, notificationType (appointment/vaccine/followup) y scheduledDate válidos, When el servidor encola la notificación, Then retorna HTTP 200 con el campo `status: "queued"` y el notificationId generado. **Escenario 2 – Usuario sin dispositivo registrado (404):** Given el developer envía la solicitud para un userId sin token de dispositivo registrado, When el servidor consulta el registro del usuario, Then retorna HTTP 404 con el mensaje "No device token found for user". **Escenario 3 – Tipo de notificación inválido (400):** Given el developer envía un notificationType no reconocido, When el servidor valida el campo, Then retorna HTTP 400 con el mensaje "Invalid notification type" y la lista de tipos aceptados. | EP08 |
| TS06 | Configurar JSON Server con datos de autenticación simulados | Como developer frontend, quiero configurar un JSON Server con datos de usuarios y tokens simulados para que el equipo pueda desarrollar el flujo de login sin depender del backend real. | **Escenario 1 – Servidor levantado correctamente:** Given el developer ejecuta el comando de inicio de JSON Server con el archivo db.json de usuarios, When el servidor arranca sin errores, Then responde en el puerto configurado y el endpoint /users devuelve la lista de usuarios simulados. **Escenario 2 – Login simulado devuelve token:** Given el developer envía una petición POST /login con credenciales definidas en db.json, When JSON Server procesa la solicitud con el middleware de autenticación simulado, Then retorna un objeto con token y perfil del usuario (dueño o profesional) para que el frontend lo almacene localmente. **Escenario 3 – Ruta protegida rechaza sin token:** Given el developer intenta acceder a un recurso protegido sin incluir el header de Authorization, When el middleware de JSON Server verifica el token, Then retorna HTTP 401 para que el frontend redirija al login. | EP09 |
| TS07 | Datos simulados de historiales clínicos y mascotas | Como developer frontend, quiero tener endpoints simulados de historiales clínicos y mascotas en JSON Server para desarrollar y probar los componentes de visualización y registro. | **Escenario 1 – GET de historial devuelve lista correcta:** Given el developer realiza una petición GET /records con un petId como filtro, When JSON Server responde con el archivo db.json, Then retorna un array de historiales clínicos filtrados por petId con todos los campos requeridos por el frontend. **Escenario 2 – POST crea un nuevo registro simulado:** Given el developer envía una petición POST /records con los campos obligatorios, When JSON Server procesa la solicitud, Then agrega el nuevo objeto al array en db.json y retorna HTTP 201 con el objeto creado incluyendo un id autogenerado. **Escenario 3 – GET de mascotas filtra por dueño:** Given el developer realiza una petición GET /pets con el parámetro ownerId, When JSON Server responde, Then retorna únicamente las mascotas cuyo ownerId coincide con el parámetro enviado. | EP09 |
| TS08 | Datos simulados de citas e inventario | Como developer frontend, quiero endpoints simulados de citas e inventario en JSON Server para que los componentes de agenda y trazabilidad puedan desarrollarse de forma independiente. | **Escenario 1 – GET de citas filtra por fecha:** Given el developer realiza una petición GET /appointments con el parámetro date, When JSON Server procesa la consulta, Then retorna únicamente las citas cuya fecha coincide con el parámetro enviado con todos los campos requeridos por el frontend. **Escenario 2 – GET de inventario incluye flag lowStock:** Given el developer realiza una petición GET /inventory, When JSON Server responde con el archivo db.json, Then retorna el listado de suministros e incluye el campo `lowStock: true` en los registros cuyo stock es menor al mínimo configurado. **Escenario 3 – DELETE de cita elimina el registro:** Given el developer realiza una petición DELETE /appointments/:id con un id existente, When JSON Server procesa la solicitud, Then elimina el objeto del array en db.json y retorna HTTP 200 confirmando la eliminación. | EP09 |
| TS09 | Datos simulados de notificaciones y configuración | Como developer frontend, quiero endpoints simulados de notificaciones y preferencias en JSON Server para desarrollar el centro de alertas y la configuración del dueño. | **Escenario 1 – GET de notificaciones filtra por userId:** Given el developer realiza una petición GET /notifications con el parámetro userId, When JSON Server procesa la consulta, Then retorna el listado de notificaciones de ese usuario ordenado por fecha descendente con el campo read incluido. **Escenario 2 – PUT actualiza preferencias de notificación:** Given el developer realiza una petición PUT /settings/:userId con las preferencias actualizadas, When JSON Server procesa la solicitud, Then actualiza el objeto en db.json y retorna HTTP 200 con las preferencias modificadas. **Escenario 3 – POST encola notificación simulada:** Given el developer realiza una petición POST /notifications con userId y notificationType válidos, When JSON Server procesa la solicitud, Then agrega la notificación al array en db.json y retorna HTTP 201 con el objeto creado incluyendo `status: "queued"`. | EP09 |
| TS10 | Implementar servicio de autenticación en Angular | Como developer frontend, quiero implementar un AuthService en Angular que gestione login, logout y almacenamiento del token JWT para centralizar la autenticación en la aplicación. | **Escenario 1 – Login exitoso almacena el token:** Given el developer llama al método login() del AuthService con credenciales válidas, When el servicio realiza la petición al endpoint de autenticación, Then almacena el token JWT en localStorage, actualiza el estado del usuario y retorna true al componente. **Escenario 2 – Token inválido fuerza logout:** Given el developer llama a un endpoint protegido y el token ha expirado, When el HTTP interceptor detecta una respuesta 401, Then llama automáticamente a logout(), limpia el token almacenado y redirige al usuario a la ruta de login. **Escenario 3 – Guard protege rutas privadas:** Given un usuario no autenticado intenta navegar a una ruta protegida del sistema, When el AuthGuard evalúa el estado de autenticación, Then redirige al usuario a /login sin mostrar el contenido de la ruta protegida. | EP10 |
| TS11 | Implementar módulo de historiales clínicos en Angular | Como developer frontend, quiero implementar el módulo de historiales clínicos en Angular con componentes de listado, creación y edición para que los profesionales gestionen registros médicos. | **Escenario 1 – Componente de listado carga historiales:** Given el profesional navega al módulo de historiales, When el componente se inicializa y llama al RecordService, Then muestra el listado de historiales de la mascota seleccionada ordenados cronológicamente de forma descendente. **Escenario 2 – Formulario de creación valida campos obligatorios:** Given el profesional deja campos obligatorios vacíos en el formulario de nuevo historial, When intenta enviar el formulario, Then el componente muestra mensajes de validación junto a cada campo faltante y no realiza la petición al servicio. **Escenario 3 – Edición de historial actualiza la vista reactivamente:** Given el profesional modifica un campo del historial y guarda los cambios, When el RecordService confirma la actualización, Then el componente de detalle refleja los cambios sin necesidad de recargar la página. | EP10 |
| TS12 | Implementar módulo de gestión de citas en Angular | Como developer frontend, quiero implementar el módulo de citas en Angular con vista de agenda diaria, formulario de agendamiento y cancelación para centralizar la gestión de turnos. | **Escenario 1 – Vista de agenda diaria carga citas del día:** Given el profesional accede al módulo de agenda, When el componente AppointmentListComponent se inicializa con la fecha del día, Then muestra las citas del día ordenadas por hora con nombre de mascota, dueño y motivo de consulta. **Escenario 2 – Formulario de nueva cita verifica disponibilidad:** Given el profesional completa el formulario de nueva cita seleccionando fecha y hora, When el AppointmentService consulta la disponibilidad del slot, Then habilita el botón de confirmación si el slot está libre, o muestra un mensaje de no disponibilidad en caso contrario. **Escenario 3 – Cancelación actualiza la agenda reactivamente:** Given el profesional confirma la cancelación de una cita desde la vista de detalle, When el AppointmentService procesa la cancelación, Then la cita desaparece de la lista de agenda y se muestra un mensaje de confirmación. | EP10 |
| TS13 | Implementar módulo de inventario en Angular | Como developer frontend, quiero implementar el módulo de inventario en Angular con listado de suministros, registro de ingresos y alertas de stock bajo para que el personal clínico controle los recursos. | **Escenario 1 – Listado muestra indicador de stock bajo:** Given el profesional accede al módulo de inventario, When el InventoryService retorna los suministros con el campo lowStock, Then el componente muestra un indicador visual en los suministros con `lowStock: true` y los ordena al inicio de la lista. **Escenario 2 – Formulario de ingreso suma stock al existente:** Given el profesional registra un suministro con nombre ya existente, When el InventoryService recibe la respuesta del backend con el stock acumulado, Then el componente actualiza la cantidad mostrada en el listado sin duplicar el registro. **Escenario 3 – Alerta de vencimiento se muestra en el panel:** Given el sistema detecta suministros con fecha de vencimiento dentro de los próximos 30 días, When el componente de alertas evalúa los datos del inventario, Then muestra un banner de advertencia en el panel principal con el listado de productos próximos a vencer. | EP10 |
| TS14 | Implementar módulo de notificaciones en Angular | Como developer frontend, quiero implementar el módulo de notificaciones en Angular con centro de alertas y configuración de preferencias para que los usuarios gestionen sus notificaciones. | **Escenario 1 – Centro de notificaciones muestra historial de 30 días:** Given el usuario accede al centro de notificaciones, When el NotificationService retorna el historial del usuario, Then el componente muestra las notificaciones de los últimos 30 días ordenadas cronológicamente con indicador de leído/no leído. **Escenario 2 – Marcar notificación como leída actualiza el badge:** Given el usuario selecciona una notificación no leída, When el NotificationService registra la acción de lectura, Then el badge del ícono de notificaciones decrementa el contador y la notificación pierde el resaltado de no leída. **Escenario 3 – Configuración de preferencias se persiste:** Given el usuario modifica sus preferencias de notificación y guarda los cambios, When el NotificationService envía la actualización al backend, Then muestra un mensaje de confirmación y las nuevas preferencias aplican a los envíos posteriores. | EP10 |
...
> El Product Backlog representa el conjunto ordenado de todas las User Stories y Technical Stories que guían el desarrollo de Pawtient. La estimación de esfuerzo utiliza la escala de Fibonacci restringida a valores ≤ 5 (1, 2, 3, 5). Las historias originalmente estimadas en 8 puntos fueron desagregadas en tareas de menor granularidad. Las Technical Stories se organizan al final por grupo: backend, fake API y frontend Angular.

| # | Story ID | Título | Descripción | Story Points |
|:---:|:---:|:---|:---|:---:|
| 1 | US22 | Propuesta de valor para clínicas | Como visitante clínica, deseo ver los beneficios del sistema en la landing page para evaluar si Pawtient se adapta a las necesidades de mi centro. | 3 |
| 2 | US23 | Propuesta de valor para dueños | Como visitante dueño de mascota, deseo entender cómo Pawtient me ayuda a cuidar mejor a mi mascota para decidir si me registro. | 3 |
| 3 | US24 | Navegación en landing page | Como visitante, deseo usar el menú de navegación para desplazarme rápidamente a cualquier sección y conocer todo el contenido disponible. | 2 |
| 4 | US25 | Preguntas frecuentes en landing page | Como visitante, deseo consultar las preguntas frecuentes para resolver dudas antes de registrarme sin contactar al equipo de soporte. | 2 |
| 5 | US04 | Registrar historia clínica – Formulario y validaciones | Como veterinario, deseo completar y validar el formulario de historia clínica con todos los campos obligatorios para evitar registros incompletos. | 3 |
| 6 | US04 | Registrar historia clínica – Guardado y acceso móvil | Como veterinario, deseo que el sistema persista la historia clínica y la muestre correctamente desde dispositivos móviles. | 2 |
| 7 | US05 | Consultar historial médico de una mascota | Como veterinario, deseo buscar y visualizar el historial completo de una mascota para tomar decisiones clínicas informadas durante la consulta. | 5 |
| 8 | US08 | Agendar cita desde la clínica | Como veterinario, deseo registrar una nueva cita en el sistema para organizar la agenda diaria sin depender de registros manuales. | 5 |
| 9 | US10 | Visualizar agenda diaria | Como veterinario, deseo ver un resumen de las citas programadas para el día en curso para organizar mi jornada de trabajo de forma eficiente. | 3 |
| 10 | US16 | Crear perfil digital de mascota | Como dueño de mascota, deseo registrar el perfil de mi mascota para centralizar su información médica y acceder a ella en cualquier momento. | 5 |
| 11 | US09 | Agendar cita como dueño de mascota | Como dueño de mascota, deseo solicitar una cita veterinaria desde mi celular para evitar llamadas telefónicas y reducir el tiempo invertido. | 5 |
| 12 | US11 | Recordatorios de citas | Como dueño de mascota, deseo recibir recordatorios automáticos antes de las citas de mi mascota para no olvidar los turnos médicos. | 3 |
| 13 | US07 | Registrar vacunas aplicadas | Como veterinario, deseo registrar cada vacuna aplicada vinculándola al historial de la mascota para mantener un calendario de vacunación actualizado. | 5 |
| 14 | US19 | Recordatorios de vacunación | Como dueño de mascota, deseo recibir alertas cuando la vacuna de mi mascota esté próxima para no perder el calendario de vacunación. | 3 |
| 15 | US12 | Registrar ingreso de suministros | Como administrador de clínica, deseo registrar el ingreso de nuevos suministros al inventario para mantener el stock actualizado en tiempo real. | 5 |
| 16 | US13 | Consultar niveles de stock | Como veterinario, deseo consultar el nivel actual de un suministro para verificar disponibilidad antes de prescribir un tratamiento. | 3 |
| 17 | US14 | Registrar consumo de suministros por consulta | Como veterinario, deseo registrar los insumos utilizados durante una consulta para descontarlos automáticamente del inventario. | 5 |
| 18 | US15 | Alertas de reabastecimiento | Como administrador de clínica, deseo recibir alertas automáticas cuando el stock de un suministro sea bajo para solicitar reposición con anticipación. | 3 |
| 19 | US06 | Editar historia clínica | Como veterinario, deseo actualizar la historia clínica de una mascota después de cada consulta para mantener la información vigente con trazabilidad de cambios. | 5 |
| 20 | US20 | Configurar preferencias de notificación | Como dueño de mascota, deseo personalizar el tipo y frecuencia de notificaciones para gestionar las alertas según mis necesidades. | 2 |
| 21 | US21 | Notificaciones de resultados y seguimiento | Como dueño de mascota, deseo recibir notificaciones cuando el veterinario actualice el estado de seguimiento de mi mascota. | 3 |
| 22 | US17 | Compartir historial con clínica | Como dueño de mascota, deseo compartir el historial de mi mascota con una nueva clínica para que el veterinario cuente con información previa relevante. | 5 |
| 23 | US18 | Buscar clínicas veterinarias cercanas | Como dueño de mascota, deseo buscar clínicas veterinarias cercanas con sus reseñas y servicios para elegir la mejor opción. | 5 |
| 24 | US01 | Registro de veterinario | Como veterinario, deseo crear una cuenta en Pawtient con mis datos profesionales para acceder a las funcionalidades del sistema. | 5 |
| 25 | US02 | Inicio de sesión | Como usuario registrado, deseo iniciar sesión con mis credenciales para acceder a las funcionalidades según mi rol. | 3 |
| 26 | US03 | Recuperar contraseña | Como usuario registrado, deseo recuperar el acceso a mi cuenta en caso de olvidar mi contraseña para no perder el acceso a la plataforma. | 3 |
| 27 | TS01 | API – Autenticación (Backend) | Como developer backend, deseo implementar el endpoint POST /api/v1/auth/login en Spring Boot que retorne un token JWT con el rol del usuario. | 3 |
| 28 | TS02 | API – Historial clínico (Backend) | Como developer backend, deseo implementar el endpoint POST /api/v1/records que persista historias clínicas en la base de datos. | 3 |
| 29 | TS03 | API – Citas (Backend) | Como developer backend, deseo implementar el endpoint POST /api/v1/appointments que registre citas verificando disponibilidad de forma transaccional. | 3 |
| 30 | TS04 | API – Inventario (Backend) | Como developer backend, deseo implementar los endpoints GET y POST /api/v1/inventory para consultar y registrar suministros con flag de stock bajo. | 3 |
| 31 | TS05 | API – Notificaciones (Backend) | Como developer backend, deseo implementar el endpoint POST /api/v1/notifications/send que encole y dispare notificaciones push. | 2 |
| 32 | TS06 | Fake API – Autenticación simulada | Como developer frontend, deseo configurar JSON Server con datos de usuarios y tokens simulados para desarrollar el flujo de login sin depender del backend. | 2 |
| 33 | TS07 | Fake API – Historiales y mascotas | Como developer frontend, deseo tener endpoints simulados de historiales clínicos y mascotas para probar los componentes de visualización y registro. | 2 |
| 34 | TS08 | Fake API – Citas e inventario | Como developer frontend, deseo endpoints simulados de citas e inventario para desarrollar los componentes de agenda y trazabilidad de forma independiente. | 2 |
| 35 | TS09 | Fake API – Notificaciones y configuración | Como developer frontend, deseo endpoints simulados de notificaciones y preferencias para desarrollar el centro de alertas. | 1 |
| 36 | TS10 | Frontend – Servicio de autenticación Angular | Como developer frontend, deseo implementar AuthService en Angular con login, logout, token storage y guard de rutas privadas. | 3 |
| 37 | TS11 | Frontend – Módulo de historiales clínicos Angular | Como developer frontend, deseo implementar el módulo de historiales en Angular con componentes de listado, creación y edición. | 5 |
| 38 | TS12 | Frontend – Módulo de gestión de citas Angular | Como developer frontend, deseo implementar el módulo de citas en Angular con agenda diaria, agendamiento y cancelación de turnos. | 5 |
| 39 | TS13 | Frontend – Módulo de inventario Angular | Como developer frontend, deseo implementar el módulo de inventario en Angular con listado, registro de ingresos y alertas de stock bajo. | 5 |
| 40 | TS14 | Frontend – Módulo de notificaciones Angular | Como developer frontend, deseo implementar el módulo de notificaciones en Angular con centro de alertas y configuración de preferencias. | 3 |
...
El diagrama de base de datos de Pawtient representa el esquema relacional del sistema, organizado en cinco módulos funcionales. El módulo de Usuarios centraliza la autenticación mediante la tabla Users, con perfiles extendidos en Pet_owners y Veterinarians. El módulo de Mascotas normaliza especie y raza en catálogos independientes (Species, Breeds) para eliminar dependencias transitivas y valores inconsistentes. El módulo de Historia Clínica estructura el expediente médico de cada mascota a través de Medical_records y Consultations, desde donde se registran signos vitales, diagnósticos, exámenes y prescripciones con catálogos controlados. El módulo de Citas gestiona la disponibilidad de veterinarios mediante Schedules y Appointments, vinculando cada cita confirmada con su consulta clínica correspondiente. Finalmente, el módulo de Inventario traza el ciclo completo desde el proveedor hasta el consumo de productos, cerrándose con Prescription_items, que conecta cada medicamento prescrito con el stock disponible.
